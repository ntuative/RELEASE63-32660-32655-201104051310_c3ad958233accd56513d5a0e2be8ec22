package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1338:String = "asset-libraries";
      
      private static const const_1334:String = "library";
      
      private static const const_1339:String = "service-libraries";
      
      private static const const_1336:String = "library";
      
      private static const const_1340:String = "component-libraries";
      
      private static const const_1333:String = "library";
      
      private static const const_1332:String = "error_data";
      
      private static const const_1337:String = "error_cat";
      
      private static const const_1335:String = "error_desc";
       
      
      private var var_370:LibraryLoaderQueue;
      
      private var var_791:IEventDispatcher;
      
      private var var_521:uint;
      
      private var var_369:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1101:
               debug("Core; using simple frame update handler");
               this.var_369 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_735:
               debug("Core; using complex frame update handler");
               this.var_369 = this.complexFrameUpdateHandler;
               break;
            case Core.const_790:
               debug("Core; using profiler frame update handler");
               this.var_369 = this.profilerFrameUpdateHandler;
               this.var_207 = new Profiler(this);
               attachComponent(this.var_207,[new IIDProfiler()]);
               var_401.addChild(new ProfilerViewer(this.var_207));
               break;
            case Core.const_474:
               debug("Core; using experimental frame update handler");
               this.var_369 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_369 = this.debugFrameUpdateHandler;
         }
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_265[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1332,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1337,String(param3));
         ErrorReportStorage.setParameter(const_1335,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_791 = loadingEventDelegate;
         if(this.var_370 == null)
         {
            this.var_370 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1338)[0];
            if(node != null)
            {
               list = node.child(const_1334);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_325,e);
         }
         try
         {
            node = config.child(const_1339)[0];
            if(node != null)
            {
               list = node.child(const_1336);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_325,e);
         }
         try
         {
            node = config.child(const_1340)[0];
            if(node != null)
            {
               list = node.child(const_1333);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_325,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1492);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_791)
         {
            this.var_791.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_791)
         {
            ...
            this.var_791.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_791)
                  {
                     this.var_791.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_521 - this.var_370.length,this.var_521,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_207 ? 0 : this.var_1468 & 0;
         if(_loc3_ == Core.const_474)
         {
            this.var_265[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_265[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_207 ? 0 : this.var_1468 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_103)
         {
            _loc3_ = this.var_265[_loc5_] as Array;
            if(_loc4_ == Core.const_474)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_369(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_103)
         {
            ...
            receivers = this.var_265[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_207)
            {
               method_6(this.var_207);
               this.var_207.dispose();
               this.var_207 = null;
            }
            switch(this.var_1468 & 0)
            {
               case Core.const_1101:
                  this.var_369 = this.simpleFrameUpdateHandler;
                  break;
