package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_372:String = "success";
      
      public static const const_338:String = "failure";
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1448 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1449.init();
            this.var_1449.addEventListener(HabboWeb.const_241,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1448;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1448);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1448);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         else
         {
            try
            {
               this.var_1819 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1820 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2486 = Boolean(facebookData[1]);
               }
               if(this.var_1819.length > 0)
               {
                  dispatchEvent(new Event(const_372));
               }
               else
               {
                  dispatchEvent(new Event(const_338));
               }
            }
            catch(e:Error)
            {
