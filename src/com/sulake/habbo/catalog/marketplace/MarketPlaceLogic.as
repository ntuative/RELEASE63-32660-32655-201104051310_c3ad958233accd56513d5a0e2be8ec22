package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1960:int = 1;
      
      public const const_1869:int = 2;
      
      public const const_2052:int = 3;
      ...
      private var var_2814:MarketplaceItemStats;
      ...
      public function dispose() : void
      {
         ...
         {
            this.disposeOffers(this.var_280);
            this.var_280 = null;
         }
      ...
      {
         this.requestOffers(this.var_2819,this.var_2818,this.var_2654,this.var_2415);
      }
      ...
      {
         if(!this.var_280 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_280.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1960,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_447)
            {
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_280.add(_loc4_.offerId,_loc5_);
         }
         if(this._visualization != null)
         {
            this._visualization.listUpdatedNotify();
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         this.var_2639 = _loc3_.creditsWaiting;
         for each(_loc4_ in _loc3_.offers)
         {
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_280.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_280.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_280.add(parser.offerId,updateItem);
            }
            this.var_280.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1869,updateItem);
            if(this._visualization != null)
            {
               this._visualization.listUpdatedNotify();
            }
      ...
      {
         if(param1.furniCategoryId != this.var_2815 || param1.furniTypeId != this.var_2816)
         {
            return;
         }
      ...
      private function isPosterItem(param1:IMarketPlaceOfferData) : Boolean
      {
         ...
         {
            _loc3_ = this._catalog.getFurnitureData(param1.furniId,ProductTypeEnum.const_72);
            if(_loc3_)
            {
               _loc4_ = _loc3_.name;
               if(_loc4_ != null && _loc4_ == const_1331)
               {
                  _loc2_ = true;
               }
            }
