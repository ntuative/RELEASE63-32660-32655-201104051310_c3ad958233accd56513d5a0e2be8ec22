package com.sulake.habbo.toolbar
{
   ...
   public class ToolbarIconAnimator
   {
      ...
      public function ToolbarIconAnimator(param1:StateItem, param2:IAssetLibrary, param3:IBitmapWrapperWindow, param4:Number, param5:Number, param6:BitmapData = null)
      {
         ...
         this.var_1733 = param1.nextState;
         this.var_1470 = param1.loop;
         if(param1.background != null)
         {
            this.var_792 = this.getBitmapDataFromAsset(param1.background,param2);
         }
         ...
         if(_loc9_.length() > 0)
         {
            ...
            while(_loc10_ < _loc9_.length())
            {
               ...
               if(_loc8_ != null)
               {
                  this.var_962.push(_loc11_);
                  this.var_963.push(_loc8_);
               }
               _loc10_++;
            }
         }
         else
         {
            ...
            if(_loc8_ != null)
            {
               this.var_962.push(new XML());
               this.var_963.push(_loc8_);
            }
         }
         if(param6 != null)
         {
            this.var_963[this._frame] = param6.clone();
         }
      ...
      {
         if(this.var_1470 == true || this.var_961 == 0)
         {
            if(this.var_962.length > 0)
            {
               ++this._frame;
               if(this._frame >= this.var_962.length)
               {
                  this._frame = 0;
                  ++this.var_961;
               }
      ...
      {
         return this.var_962.length;
      }
      
      public function hasNextState() : Boolean
      {
         return this.var_1470 == false && this.var_961 == 1 && this.var_1733 != this._state;
      }
      ...
      {
         var _loc2_:* = null;
         if(this._frame <= this.var_963.length)
         {
            _loc2_ = this.var_963[this._frame];
      ...
      private function setAsset(param1:IBitmapWrapperWindow, param2:BitmapData) : void
      {
         ...
         {
            _loc3_ = new BitmapData(Math.max(param2.width,this.var_792.width),Math.max(param2.height,this.var_792.height),true,0);
            _loc4_ = _loc3_.height - this.var_792.rect.height;
            _loc3_.copyPixels(this.var_792,this.var_792.rect,new Point(0,_loc4_),null,null,true);
