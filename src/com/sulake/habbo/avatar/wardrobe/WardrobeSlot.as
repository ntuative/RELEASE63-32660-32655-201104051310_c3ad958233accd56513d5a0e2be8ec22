package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_385 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_657 && this.var_1528)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_90,this.var_1003,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_37,parseInt(FigureData.const_838));
         ...
         if(this.var_385)
         {
            if(this.var_385.bitmap)
            {
               this.var_385.bitmap.dispose();
            }
            this.var_385.bitmap = new BitmapData(this.var_385.width,this.var_385.height,true,0);
            _loc7_ = (this.var_385.width - _loc1_.width) / 2;
            _loc8_ = (this.var_385.height - _loc1_.height) / 2;
            this.var_385.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
         ...
         if(_loc4_)
         {
            _loc4_.visible = this.var_1528 && this.var_657 != null;
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_657 = this._controller.figureData.getFigureString();
               this.var_1003 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1973,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_657)
               {
                  this._controller.loadAvatarInEditor(this.var_657,this.var_1003,this._controller.clubMemberLevel);
               }
         }
      }
