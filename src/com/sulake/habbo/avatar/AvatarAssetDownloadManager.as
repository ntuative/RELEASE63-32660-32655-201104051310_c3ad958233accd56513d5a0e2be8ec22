package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1276:Dictionary;
      ...
      private var var_462:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_462 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1032 = new Timer(this.const_2043,1);
         this.var_1032.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_462)
         {
            this.var_462.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_462.addEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_462)
         {
            this.var_462.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_462.removeEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1558.indexOf("?") > 0)
            {
               _loc2_ = this.var_1558 + "&retry=" + this.var_1031;
            }
            else
            {
               _loc2_ = this.var_1558 + "?retry=" + this.var_1031;
            ...
            this.var_462 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2809,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1276[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_532[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_532[_loc3_] = _loc6_;
            }
            this.var_1033[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1033)
         {
            _loc7_ = true;
            _loc4_ = this.var_1033[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_532[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1033[_loc3_];
         }
         while(_loc6_ < this.var_843.length)
         {
            _loc5_ = this.var_843[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_843.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1032.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_842.length > 0 && this.var_843.length < this.const_1538)
         {
            _loc1_ = this.var_842[0];
            _loc1_.startDownloading();
            this.var_843.push(this.var_842.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_842.indexOf(param1) == -1 && this.var_843.indexOf(param1) == -1)
         {
