package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_2016 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1278 = this.var_2016.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1278)
         {
            this.var_1278.procedure = this.onButtonClick;
         }
         this.var_1277 = this.var_2016.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1277)
         {
            this.var_1277.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_281 >= this.var_66.length - 1)
         {
            this.var_1277.disable();
         }
         else
         {
            this.var_1277.enable();
         }
         if(this.var_281 <= 0)
         {
            this.var_1278.disable();
         }
         else
         {
            this.var_1278.enable();
         }
      }
      ...
      {
         ++this.var_281;
         this.var_281 = Math.min(this.var_281,this.var_66.length - 1);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_82.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_66.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_66 || this.var_66.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_66[this.var_281][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_66[this.var_281][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_66[this.var_281][1]).value;
